export async function onRequestPost({ request, env }) {
  try {
    ...
    await env.DB.prepare('UPDATE magic_links SET used_at = datetime("now") WHERE id = ?').bind(link.id).run()

    // Upsert user
    await env.DB.prepare('INSERT OR IGNORE INTO users (email) VALUES (?)').bind(link.email).run()
    const { results: users } = await env.DB.prepare('SELECT * FROM users WHERE email = ?').bind(link.email).all()
    const user = users[0]
